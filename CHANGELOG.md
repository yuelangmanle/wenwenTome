...
## [2.7.5] - 2026-03-26 「Android 稳定性回撤」
### 修复与优化
- Android 本地 `EPUB / TXT` 默认回退到稳定 Flutter 阅读器，不再把 Foliate localhost 宿主作为默认主链路。
- Android 网文全站搜书改为优先使用稳定内置源，避免半兼容导入源占满移动端默认候选导致“搜不到”。
- 保留 Foliate / Legado 迁移代码，但不再把未完成的嵌入链路当成默认用户路径。
